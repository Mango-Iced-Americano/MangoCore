...
| la64 `getrusage03` 变成 `TCONF: needs at least 512MB MemAvailable` 或 30s timeout | la64 高段 RAM 只有 768MB，256MB 静态 kernel heap 挤占可用页帧；页帧清零被降成 byte-wise/volatile store，大批量 fault 很慢 | la64 kernel heap 保持在实际需要范围内；页帧清零用普通 64-bit store 循环，避免每页 4096 次 byte store 或 volatile 写 |
| la64 大量 page fault/syscall 场景明显慢 | trap 入口未切换 PGDL 却无条件 `invtlb`，返回用户态也不判断 PGDL 是否变化；每次 trap 都额外全刷 TLB | 入口不 flush；`__restore` 仅在 PGDL 改变时刷新，PTE 修改路径仍各自刷新 |
...
| futex 压力后 `getrusage03`/fork 变慢 | 内部 `RecycleAllocator`（用户资源 slot/kstack）释放仍线性扫描 recycled Vec；大量 clone/exit 后形成 O(n²) 查重 | `RecycleAllocator` 维护 recycled membership bitmap，alloc/dealloc 都 O(1) 更新 |
| futex 压力后 la64 glibc `getrusage03` 在 `clone` panic `address_space.rs` | fork 复制地址空间时用 `last_non_user()` 猜 trap context，clone/exit churn 后可能选到错误或无 PTE 的非用户 VMA | `from_existing_user()` 由当前 task 的 `user_res_slot` 精确定位 trap context，并直接复制当前 `TrapContext` 内容 |
...
| futex/getrusage 压测后 PCB/zombie 对象长期不回落 | `wait_child()` 消费 zombie 后只从父进程 `children` 摘链并释放 pid，未清子进程 `parent`，也未从 process registry 删除；auto-reap 路径未完整释放 pid/parent；父进程退出时已 zombie 子进程被简单转交 init，容易残留 | wait 消费时同步 `release_pid`、清 `parent`、`unregister_process`；`SIGCHLD=SIG_IGN`/`SA_NOCLDWAIT` auto-reap 只丢弃状态并释放对象，不计入父进程 `RUSAGE_CHILDREN`；父进程退出时 live child 才转交 init，已 zombie orphan 直接释放并把 rusage 归到 init |
