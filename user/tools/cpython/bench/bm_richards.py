# Adapted from python/pyperformance bm_richards at
# 216cbeb5f828b8ee5864f9bb52f3563d2d1a4846 (MIT).
# See PYPERFORMANCE_LICENSE.txt in this directory.
...
class Richards(object):
    ...
    def run(self, iterations):
        for i in range(iterations):
            # pyperformance uses a module-global work area. Reset its task
            # chain as well as its counters so warmups cannot lengthen later
            # samples.
...
def benchmark():
    richards = Richards()
    if not richards.run(1):
        raise RuntimeError("Richards validation counters mismatch")
