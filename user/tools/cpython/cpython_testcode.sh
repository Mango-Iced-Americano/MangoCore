...
cd /tools/tests/cpython || {
    echo "[CPYTHON L0 FAIL] missing /tools/tests/cpython"
...
# Keep the read-only tools partition immutable on the real board. QEMU tools
# disks are writable, while staged 2K1000 images provide writable FAT32 at
# /scratch; fall back to tmpfs when neither persistent location is writable.
if [ -d /scratch ] && /bin/busybox mkdir -p /scratch/cpython; then
...
elif /bin/busybox touch /tools/tests/cpython/.write-probe 2>/dev/null; then
    /bin/busybox rm -f /tools/tests/cpython/.write-probe
    CPYTHON_TEST_TMPDIR=/tools/tests/cpython
else
...
run_sh() {
    ...
    if "/tools/tests/cpython/$name"; then
        rc=0
    else
        rc=$?
    fi
...
run_py() {
    name="$1" layer="$2"
    echo "[CPYTHON $layer START] $name"
    if "$CPYTHON_LD" --library-path "$LD_LIBRARY_PATH" "$CPYTHON_PY" "/tools/tests/cpython/$name"; then
        rc=0
    else
        rc=$?
    fi
