...
pub fn run_ltp_suite_runner(environ: &[*const u8], root: &str, suffix: &str, timeout: u64, conf: Option<&[u8]>) {
    ...
    if pid == 0 { let root = format!("{}\0", root); let timeout = format!("{}\0", timeout.saturating_sub(50)); let conf = conf.unwrap_or(b"/os_test.conf\0"); exec("/ltprunner\0", &["/ltprunner\0".as_ptr(), "--conf\0".as_ptr(), conf.as_ptr(), "--libc\0".as_ptr(), format!("{}\0", suffix).as_ptr(), "--ltproot\0".as_ptr(), root.as_ptr(), "--group-timeout-secs\0".as_ptr(), timeout.as_ptr(), core::ptr::null()], environ); exit(127); }
