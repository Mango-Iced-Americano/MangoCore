...
pub fn run_ltp_binaries(environ: &[*const u8], dir: &str, exclude: &[String], include: &[String], from: Option<&str>, timeout: u64) {
    ...
    let child = fork();
    if child == 0 {
        let chdir_result = chdir(dir);
        if chdir_result < 0 { println!("FAIL LTP setup: chdir {} returned {}", root, chdir_result); exit(126); }
        let fd = open("ltp/testcases/bin\0", OpenFlags::RDONLY);
        if fd < 0 { println!("FAIL LTP setup: open {}/ltp/testcases/bin returned {}", root, fd); exit(126); }
        if !include.is_empty() {
            for name in include {
                let command = format!("export LTPROOT={}/ltp; export LTP_IPC_PATH=/tmp; export PATH={}/ltp/testcases/bin:$PATH; ./ltp/testcases/bin/{}\0", root, root, name);
                let code = exit_code(run_bash_cmd_timeout(&command, environ, 30));
                println!("{} LTP CASE {} : {}", if code == 0 { "DONE" } else { "FAIL" }, name, code);
            }
    ...
    if child < 0 { println!("FAIL LTP setup: fork returned {}", child); return; }
    let mut status = 0; let _ = waitpid(child as usize, &mut status); let _ = timeout; println!("#### OS COMP TEST GROUP END ltp-{} ####", suffix);
}
