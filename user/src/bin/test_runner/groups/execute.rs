...
pub fn run_selected_groups(environ: &[*const u8], cfg: &RuntimeConfig) {
    ...
    for &index in &cfg.order { let (group, script) = TEST_GROUPS[index]; if cfg.mask & (1 << index) == 0 { println!("[initproc] skip {} (mask bit{} not set)", group, index); continue; }
        ...
        else if group == "ltp" && cfg.ltp_runner == LtpRunner::Suite { run_ltp_suite_runner(environ, "/musl", "musl", cfg.timeouts[index], cfg.conf_source.as_deref()); run_ltp_suite_runner(environ, "/glibc", "glibc", cfg.timeouts[index], cfg.conf_source.as_deref()); }
