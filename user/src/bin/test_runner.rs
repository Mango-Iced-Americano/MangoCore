...
use alloc::vec::Vec;
use user_lib::{
    chdir, close, exec, exit, fork, get_time, getdents64, getpgid, getpid, getppid, kill, mount, open, println,
...
fn try_mount(source: &str, target: &str, fstype: &str, flags: usize) -> isize {
    let source = format!("{}\0", source);
    let target = format!("{}\0", target);
    let fstype = format!("{}\0", fstype);
    mount(source.as_ptr(), target.as_ptr(), fstype.as_ptr(), flags, 0)
}

fn wait_child_bounded(pid: isize) -> bool {
    let mut status = 0;
    ...
    loop {
        ...
        if waited < 0 || (get_time() as usize).saturating_sub(start) >= NTP_TIMEOUT_MS {
            let _ = kill(pid as usize, SIGKILL);
            let _ = waitpid(pid as usize, &mut status);
            return false;
        }
        sleep(50);
    }
...
fn try_ntp_sync() {
    ...
    if pid == 0 {
        ...
        exec(shell, &args, &[core::ptr::null()]);
        exit(127);
    }
    if pid > 0 && wait_child_bounded(pid) {
        println!("[test-runner] ntpd time sync ok");
        return;
    }
    let fallback = TimeSpec { tv_sec: 1749049200, tv_nsec: 0 };
    let _ = sys_clock_settime(0, &fallback);
    println!("[test-runner] ntpd unavailable; using fallback clock");
}

fn bootstrap_compat() {
    println!("[test-runner] bootstrap compatibility setup");
    try_ntp_sync();
    for (source, target) in [
        ...
        let _ = sys_mkdirat(AT_FDCWD, format!("{}\0", target).as_str(), 0o755);
        let _ = try_mount(source, target, "", MS_BIND);
    }
}
...
fn main(_argc: usize, _argv: &[&str]) -> i32 {
    if getpid() == 1 || getppid() != 1 {
        println!(
            "[test-runner] lifecycle violation pid={} ppid={}; expected non-PID1 child of init",
            getpid(),
            getppid()
        );
        return 1;
