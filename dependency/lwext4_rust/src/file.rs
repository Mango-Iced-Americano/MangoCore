    ...
    pub fn lwext4_dir_entries(&self) -> Result<(Vec<Vec<u8>>, Vec<InodeTypes>), i32> {
        ...
        unsafe {
            ...
            while !de.is_null() {
                ...
                // Allocate len+1 to avoid overflow when name_length == 255
                let mut sss: alloc::vec::Vec<u8> = alloc::vec![0u8; len + 1];
                sss[..len].copy_from_slice(&dentry.name[..len]);
                // sss[len] is already 0 from vec initialization

                debug!(
                    "  {} {}",
                    dentry.inode_type,
                    core::str::from_utf8(&sss[..len]).unwrap()
                );
                name.push(sss);
