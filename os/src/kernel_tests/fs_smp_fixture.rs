...
use crate::{
    ...
    fs::{
        tmpfs::TmpFS,
        vfs::{BackendLifecycle, File, FileFlags, FilePrivateData, FileSystem, FileType, IndexNode, InodeMode, Metadata, MountFS, MountFSInode, MountFlags},
        PageCache, PageCacheBackend, PageCacheTestHook,
    },
    kernel_tests::probe::{attach_probe_to_runner, build_user_probe, deadline_after, probe_quiesced, reap_probe, stop_probe, ProbeResult},
...
impl MountedTmpfsFixture {
    pub(crate) fn mount() -> Result<Self, &'static str> {
        let target = crate::fs::vfs_lookup_absolute("/dev/shm").map_err(|_| "ktest has no /dev/shm mountpoint")?;
        let target = target
            .as_any_ref()
        ...
        let fs = TmpFS::new();
        let mount = target
            .mount_subtree(BackendLifecycle::new(fs.clone()), fs.root_inode(), MountFlags::empty(), Some("/dev/shm".into()))
...
impl IndexNode for FsSmpCacheInode {
    fn read_at(
        ...
        self.cache.read_kernel(offset, &mut buf[..len.min(file_size - offset)])
    }

    fn write_at_user(&self, offset: usize, len: usize, src: &crate::mm::UserBuffer) -> Result<usize, SyscallErr> {
...
pub(crate) fn new_cache(hook: Option<PageCacheTestHook>) -> Arc<PageCache> {
    let cache = PageCache::new_with_test_hook(hook);
    cache.set_backend(Arc::new(FsSmpPageBackend::new()));
    cache
}

pub(crate) fn read_inode(inode: &Arc<dyn IndexNode>, offset: usize, data: &mut [u8]) -> Result<usize, SyscallErr> {
...
pub(crate) fn run_dual_user_writes(
    ...
    let left_file = File::new(inode.clone(), FileFlags::O_WRONLY).map_err(|_| "failed to create CPU1 writer fd")?;
    left_file.set_offset(left_offset);
    let right_file = File::new(inode, FileFlags::O_WRONLY).map_err(|_| "failed to create CPU2 writer fd")?;
