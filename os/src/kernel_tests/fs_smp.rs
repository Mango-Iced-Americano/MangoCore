...
fn tmpfs_create_same_name_exactly_once() -> Result<(), &'static str> {
    ...
    let first_clean = probe_quiesced(&first, &first.process, 1, deadline_after(3)) || stop_probe(&first, &first.process, 1);
    let second_clean = probe_quiesced(&second, &second.process, 2, deadline_after(3)) || stop_probe(&second, &second.process, 2);
...
fn tmpfs_cross_rename_opposite_order() -> Result<(), &'static str> {
    ...
    directory.create("ktest_rename_a", FileType::File, InodeMode::S_IRWXUGO).map_err(|_| "failed to create tmpfs rename source A")?;
    directory.create("ktest_rename_b", FileType::File, InodeMode::S_IRWXUGO).map_err(|_| "failed to create tmpfs rename source B")?;
    ...
    if wait_status_exit_code(first.process.exit_code()) != 0 || wait_status_exit_code(second.process.exit_code()) != 0 {
...
fn pagecache_user_write_vs_truncate() -> Result<(), &'static str> {
    ...
    let writer = build_user_probe(ProbeResult::WritePage, writer_file, 64, Some(&[0xa5; PAGE_SIZE]))?;
    ...
    let truncater = match build_user_probe(ProbeResult::Zero, truncater_file, SYSCALL_FTRUNCATE, None) {
        Ok(task) => task,
        Err(error) => {
            HOOK_PHASE.store(PHASE_RELEASE_WRITER, Ordering::Release);
            let stopped = stop_probe(&writer, &writer.process, 1);
