...
const FAT_WRITER_SIZE: usize = 2048;
...
// a0 = 路径指针。循环 N 次：openat(O_CREAT|O_WRONLY|O_TRUNC) → ftruncate(fd, 2048)
// → write(fd, 路径页, 2048) → close。每次迭代 ftruncate 都从 FAT 分配 4 个连续簇，
// 下轮 openat 的 O_TRUNC 又把它们释放，形成持续 alloc/free 竞争。退出码区分阶段：
// 1=openat、2=ftruncate、3=write 短写。
#[cfg(target_arch = "riscv64")]
core::arch::global_asm!(
    r#"
...
// a0 = 路径指针。循环 N 次：openat(O_CREAT|O_WRONLY|O_TRUNC) → ftruncate(2048)
// [分配 4 簇] → ftruncate(512) [释放 3 簇] → close → unlinkat [inode drop 释放
// 剩余 1 簇]。与 writer 探针并发时在同一簇表上做 alloc+free 竞争。退出码区分阶段：
// 1=openat、2=ftruncate、3=unlinkat。
#[cfg(target_arch = "riscv64")]
core::arch::global_asm!(
    r#"
...
"#,
    ...
    ftruncate_syscall = const SYSCALL_FTRUNCATE,
    close_syscall = const SYSCALL_CLOSE,
    unlinkat_syscall = const SYSCALL_UNLINKAT,
    exit_syscall = const SYSCALL_EXIT,
);
...
pub(crate) enum ProbeResult {
    ...
    EventFdWrite,
    EpollEdge,
    /// FAT 簇表并发 writer：openat → ftruncate(2048) → write → close，循环。
    FatAllocWriter,
    /// FAT 簇表并发 alloc/free：openat → ftruncate(2048) → ftruncate(512) →
    /// close → unlinkat，循环。
    FatAllocFree,
}
