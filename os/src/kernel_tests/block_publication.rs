...
fn test_publishes_names_independent_of_descriptor_order() -> Result<(), &'static str> {
    // Given: named descriptors in reverse order.
    let first = descriptor("vda", 0)?;
    let second = descriptor("vdb", 1)?;
    let mut registry = BootBlockRegistry::new();

    // When: the registry publishes both descriptors.
    registry
        .register_all(&[second, first])
        .map_err(|_| "descriptor publication failed")?;
    // Then: lookup follows names rather than descriptor slots.
...
fn test_rejects_duplicate_names_and_device_numbers() -> Result<(), &'static str> {
    ...
    let first = descriptor("vda", 0)?;
    let same_name = descriptor("vda", 1)?;
    let same_number = descriptor("vdb", 0)?;
    let mut registry = BootBlockRegistry::new();
...
fn test_devfs_uses_descriptor_name_and_device_number() -> Result<(), &'static str> {
    ...
    let descriptor = descriptor("vdc", 7)?;
    let devfs = DevFS::new();
...
fn descriptor(name: &str, minor: u64) -> Result<BlockDeviceDescriptor, &'static str> {
