...
pub(crate) const MFD_VALID_FLAGS: u32 = MFD_CLOEXEC | MFD_ALLOW_SEALING | MFD_HUGETLB | MFD_HUGE_MASK;
...
pub(crate) fn metadata_to_statx(meta: &vfs::Metadata, mask: u32) -> Statx {
...
pub(crate) fn open_proc_self_fd(path: &str, flags: OpenFlags) -> Option<Result<Arc<vfs::File>, isize>> {
...
pub(crate) fn pread_into_user(file: &vfs::File, token: usize, buf: usize, count: usize, offset: usize) -> isize {
    if count == 0 {
...
pub(crate) fn pwrite_from_user(file: &vfs::File, token: usize, buf: usize, count: usize, offset: usize) -> isize {
    if count == 0 {
        return 0_isize;
    }
...
pub(crate) fn splice_write_stream(file: &vfs::File, buf: &[u8], nonblock: bool) -> Result<usize, isize> {
...
pub(crate) fn resolve_umount_target(inode: &Arc<dyn vfs::IndexNode>) -> Result<Arc<vfs::MountFS>, isize> {
...
pub(crate) fn record_flock_close(
...
pub(crate) fn resolve_path_inode(path: &str, follow_final: bool) -> Result<Arc<dyn vfs::IndexNode>, isize> {
