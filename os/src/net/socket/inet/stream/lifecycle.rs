...
impl Inner {
    ...
    pub fn connect(
        ...
        if let Err(e) = route_check(remote_endpoint.addr) {
            log::info!(
                "[tcp::connect] route_check failed for {:?}: {:?}",
    ...
    pub fn listen(self, backlog: usize) -> Result<Listening, (Self, SyscallErr)> {
        ...
        let listen_ifindex = crate::net::net_core::ifindex_for_local_addr(listen_addr.addr);
        ...
        for _ in 1..backlog {
            ...
            if let Some(h) =
                NET_INTERFACE.add_routed_socket_on(InetProtocol::Tcp, new_socket, listen_ifindex)
            {
                handles.push(h);
            }
        }
