...
use super::netlink::{
    ARPHRD_ETHER, ARPHRD_LOOPBACK, IFA_ADDRESS, IFA_LABEL, IFA_LOCAL, IFLA_ADDRESS, IFLA_IFNAME,
    IFLA_MTU, NDA_DST, NDA_LLADDR, NLMSG_DONE, NLMSG_ERROR, NLM_F_DUMP, NLM_F_MULTI, NLM_F_REQUEST,
    NLM_F_ROOT, RTA_DST, RTA_GATEWAY, RTA_OIF, RTM_DELADDR, RTM_DELLINK, RTM_DELNEIGH,
...
use super::segment::{
    CMsgSegHdr, DoneSegment, DoneSegmentBody, ErrorSegment, ErrorSegmentBody, NoAttr,
    RouteNlSegment, SegmentCommon,
};
...
pub fn handle_netlink_msg(
    ...
    let (msg_type, flags, seq, _req_pid) = match parse_nlmsg(buf) {
        ...
        None => {
            log::error!(
                "[netlink] handle_netlink_msg: failed to parse nlmsghdr (len={})",
                buf.len()
            );
            return Ok(0);
        }
    ...
    log::warn!(
        "[netlink] handle msg type={} flags={:#x} seq={} pid={}",
        msg_type,
        flags,
        seq,
        _req_pid
    ...
    if flags & NLM_F_REQUEST == 0 {
        log::error!(
            "[netlink] ignoring non-request message (flags={:#x})",
            flags
        );
        return Ok(0);
    }
    ...
    if !is_get || !is_dump {
        ...
        if let Err(e) = result {
            let errno = e as i32;
            log::warn!(
                "[netlink] handler for msg_type={} failed: {:?} (errno={})",
                msg_type,
                e,
                errno
            );
    ...
    if let Err(e) = result {
        let errno = e as i32;
        log::warn!(
            "[netlink] dump handler for msg_type={} failed: {:?} (errno={})",
            msg_type,
            e,
            errno
        );
...
fn handle_getlink(
    ...
    for (nic_id, iface) in list.iter() {
        ...
        let mut n = Vec::new();
        n.extend_from_slice(iface.iface_name().as_bytes());
        n.push(0);
        payload.extend(&super::netlink::rta_data(IFLA_IFNAME, &n));
        payload.extend(&super::netlink::rta_data(
            IFLA_MTU,
            &(iface.mtu() as u32).to_ne_bytes(),
        ));
        if iface.kind() != DeviceKind::Loopback {
            payload.extend(&super::netlink::rta_data(IFLA_ADDRESS, &iface.mac()));
        }
        if !sock.push_recv(super::netlink::build_nlmsg(
            RTM_NEWLINK,
            NLM_F_MULTI,
            seq,
            pid,
        ...
        for cidr in &iface.ip_addrs() {
            match cidr.address() {
                IpAddress::Ipv4(addr) => {
                    ...
                    label.extend_from_slice(iface.iface_name().as_bytes());
                    label.push(0);
                    attrs.extend(&super::netlink::rta_data(IFA_LABEL, &label));
                    payload.extend(&attrs);
                    out.extend(&super::netlink::build_nlmsg(
                        RTM_NEWADDR,
                        NLM_F_MULTI,
                        seq,
                        pid,
...
fn handle_getroute(
    ...
    let entries = crate::net::net_core::current_netns()
        .router
        .lock()
        .table
        .entries
    ...
    for entry in &entries {
        ...
        let mut payload = Vec::new();
        let rt = match entry.route_type {
            crate::net::routing::RouteType::Default => 3u8,
            _ => 1u8,
        };
        payload.push(2); // AF_INET
        ...
        if entry.destination.prefix_len() > 0 {
            if let IpAddress::Ipv4(a) = entry.destination.address() {
                attrs.extend(&super::netlink::rta_data(RTA_DST, &a.0));
            }
        }
        if let Some(nh) = entry.next_hop {
            if let IpAddress::Ipv4(a) = nh {
                attrs.extend(&super::netlink::rta_data(RTA_GATEWAY, &a.0));
            }
        }
        attrs.extend(&super::netlink::rta_data(
            RTA_OIF,
            &entry.ifindex.to_ne_bytes(),
        ));
        payload.extend(&attrs);
        if !sock.push_recv(super::netlink::build_nlmsg(
            RTM_NEWROUTE,
...
fn parse_nda_attrs(
    payload: &[u8],
) -> Result<(Option<IpAddress>, Option<EthernetAddress>), crate::utils::error::SyscallErr> {
