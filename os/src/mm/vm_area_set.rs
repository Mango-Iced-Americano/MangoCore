...
impl VmAreaSet {
    ...
    pub(super) fn with_capacity(capacity: usize) -> Self {
        Self {
            areas: Vec::with_capacity(capacity),
        }
    }
    ...
    pub(super) fn iter(&self) -> core::slice::Iter<'_, MapArea> {
        self.areas.iter()
    }

    pub(super) fn iter_mut(&mut self) -> core::slice::IterMut<'_, MapArea> {
        self.areas.iter_mut()
    }

    pub(super) fn get(&self, idx: usize) -> Option<&MapArea> {
        self.areas.get(idx)
    }

    pub(super) fn get_mut(&mut self, idx: usize) -> Option<&mut MapArea> {
        self.areas.get_mut(idx)
    }

    pub(super) fn last(&self) -> Option<&MapArea> {
        self.areas.last()
    }
    ...
    pub(super) fn try_reserve(&mut self, additional: usize) -> Result<(), isize> {
        ...
        self.areas.try_reserve(additional).map_err(|_| ENOMEM)
    }

    pub(super) fn push(&mut self, area: MapArea) -> Result<(), isize> {
    ...
    pub(super) fn remove(&mut self, idx: usize) -> MapArea {
        self.areas.remove(idx)
    }

    pub(super) fn find_index(&self, vpn: VirtPageNum) -> Option<usize> {
        self.areas.iter().position(|area| area.vm_contains(vpn))
    }
    ...
    pub(super) fn insert_ordered(&mut self, new_area: MapArea) -> Result<(), isize> {
    ...
    pub(super) fn remove_area_with_start<T: PageTable>(
        ...
        if let Some(idx) = self.areas.iter().position(|area| area.vm_start() == start_vpn) {
            let result = self.areas[idx].unmap(page_table);
            self.areas.remove(idx);
            result
        } else {
    ...
    pub(super) fn split_for_range(
        ...
        let area_start_vpn = self.areas[idx].vm_start();
        let area_end_vpn = self.areas[idx].vm_end();
    ...
    pub(super) fn unmap_range<T: PageTable>(
        ...
        while idx < self.areas.len() {
            if !self.areas[idx].vm_overlaps(start_vpn, end_vpn) {
                idx += 1;
                continue;
            }
            if !self.areas[idx].vm_is_user() {
                return Err(EINVAL);
            }
            found_area = true;
            let area_start_vpn = self.areas[idx].vm_start();
            ...
            if self.areas[target_idx].unmap(page_table).is_err() {
    ...
    pub(super) fn protect_range<T: PageTable>(
        ...
        while cursor < end_vpn {
            ...
            let area_end = self.areas[idx].vm_end();
...
impl Index<usize> for VmAreaSet {
    type Output = MapArea;
...
impl IndexMut<usize> for VmAreaSet {
