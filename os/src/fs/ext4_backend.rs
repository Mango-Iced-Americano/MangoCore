...
pub fn open(
    ...
    read_only: bool,
) -> Result<Arc<dyn FileSystem>, SyscallErr> {
    #[cfg(all(
        feature = "ext4_lwext4_backend",
    ...
    ))]
    {
        ...
        let filesystem: Arc<dyn FileSystem> =
            super::ext4_lwext4::ext4fs::Ext4FileSystem::open_ext4rs_with_options(
                block_device,
                read_only,
            )?;
        Ok(filesystem)
    ...
    ))]
    {
        ...
        let filesystem: Arc<dyn FileSystem> =
            super::ext4_another::Ext4FileSystem::open_with_options(block_device, read_only)?;
        Ok(filesystem)
    }
