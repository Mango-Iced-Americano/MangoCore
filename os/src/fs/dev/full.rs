...
/// Data Sink — Always Full
/// Writes to `/dev/full` always return ENOSPC (No space left on device).
/// Reads from `/dev/full` always return zero-filled bytes (like /dev/zero).
#[derive(Debug)]
pub struct Full;

impl IndexNode for Full {
    fn read_at(
        &self,
        _offset: usize,
        _len: usize,
