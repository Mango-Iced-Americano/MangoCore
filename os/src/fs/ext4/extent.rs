    ...
    pub fn try_load_from_data(
        ...
        if is_root {
            ...
            for (i, chunk) in data.chunks_exact(4).enumerate() {
                root_data[i] = u32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]);
            }
...
impl Ext4FileSystem {
    ...
    pub fn find_extent(
        ...
        while depth > 0 {
            ...
            if let Some(pos) = index_pos {
                ...
                let next_block = index.leaf_lo;
                let mut next_data = self.read_metadata_block(next_block as usize);
                node = ExtentNode::try_load_from_data_mut(&mut next_data, false, self.block_size)?;
    ...
    fn merge_extent(
        ...
        if header.max_entries_count > 4 {
            ...
            let mut ext4block = self.load_metadata_block(block);
            ...
            self.store_metadata_block_dirty(block, &ext4block.data);
    ...
    fn insert_new_extent(
        ...
        if depth == 0 {
            ...
            *inode_ref.inode.root_extent_mut_at(node.position + 1) = *new_extent;
            ...
            let mut ext4block = self.load_metadata_block(node_block);
            let new_ex_offset = core::mem::size_of::<Ext4ExtentHeader>()
            ...
            *ex = *new_extent;
            ...
            // sync to disk
            self.store_metadata_block_dirty(node_block, &ext4block.data);
            super::counters::inc_counter!(super::counters::OTHER_META_WRITE);
    ...
    fn ext_grow_indepth(&self, inode_ref: &mut Ext4InodeRef) -> Result<(), isize> {
        ...
        // load new block
        let mut new_ext4block = self.load_metadata_block(new_block as usize);
        ...
        let space = (self.block_size - core::mem::size_of::<Ext4ExtentHeader>())
...
impl Ext4FileSystem {
    ...
    pub fn extent_remove_space(
        ...
        let Some(mut ex) = search_path.path[depth].extent else {
            // No extent at this position (sparse hole) — nothing to remove
            return Ok(0);
        };
        ...
        while i >= 0 {
            ...
            if i as usize == depth {
                ...
                let ext4block = self.load_metadata_block(node_pblock);
    ...
    pub fn ext_remove_leaf(
        ...
        let mut ext4block = if node_disk_pos == 0 {
            ...
            self.load_metadata_block_offset(node_disk_pos)
        };
    ...
    fn ext_remove_idx(
        ...
        if path.path[i].position != header.entries_count as usize - 1 {
            ...
            let mut ext4block = self.load_metadata_block_offset(node_disk_pos);
        ...
        // 释放索引块
        let idx_ref = path.path[i].index.as_mut().ok_or_else(|| {
            log::error!("ext_remove_idx: index is None at depth={}", i);
            -(SyscallErr::EIO as isize)
        })?;
        self.ext_remove_index_block(inode_ref, idx_ref);
        ...
        while i > 0 {
            ...
            let current_index = right[0].index.ok_or_else(|| {
                log::error!("ext_remove_idx: current index is None at depth={}", i);
                -(SyscallErr::EIO as isize)
            })?;
    ...
    pub fn more_to_rm(&self, path: &ExtentPathNode, to: u32) -> bool {
        ...
        if let Some(index) = path.index {
            ...
            let ext4block = self.load_metadata_block_offset(node_disk_pos);
