...
pub fn ppoll(
        ...
        .is_err()
    {
        ...
        if nfds == 0 {
            ...
            // timeout=0 仍要观察调用前已经到达的网络状态。该内部 ticket 等待不使用
            // 用户 deadline，且此处尚未取得 fd table 或任一 File/private 锁。
            if timeout_is_zero {
...
pub fn pselect(
    ...
    // 与 ppoll 一样，零超时的首扫可等待一张内部 poll ticket，避免只看到 worker
    // 尚未消费的旧 pollee 状态。后续阻塞重扫仍完全由事件队列驱动。
