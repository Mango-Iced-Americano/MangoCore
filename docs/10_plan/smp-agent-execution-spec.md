...
当人工或 AI 审查发现相似命名堆叠、调用链过深、结构体职责重叠、文件拆分缺乏明确
所有权时，不能只做局部改名。先读取 Linux、DragonOS 等成熟内核的对应生产路径；涉及
指令、固件 ABI 或内存模型时，先读取官方规范。对照结果必须落实为：

1. 一个功能只保留一条可叙述的主调用链；
2. 类型和文件对应独立生命周期、所有权或并发边界，纯转发 wrapper 不单独成层；
3. 命名优先使用项目既有术语和成熟内核通用术语，同层函数保持同一动词体系；
4. 不机械复制上游与 MangoCore 架构不匹配的抽象；汇报中说明采纳与拒绝的理由；
5. 重构后重新核对注释、架构文档和测试名称，旧术语只能保留在明确标注的历史记录中。
