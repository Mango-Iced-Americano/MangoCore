...
- B61 消除普通 `msgrcv` 的重复领取窗口：消息选择和 `VecDeque::remove(idx)` 在同一个
  ...
  保持 RV64 312/314、LA64 308/314。精确双 receiver 动态竞态为 NOT RUN，IPC ID ABA 与
  WaitQueue/对象删除协议仍留在后续节点；
