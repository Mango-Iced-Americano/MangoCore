...
- B46 把 `sys_sigreturn()` 改为锁内快照 SP、锁外读取完整用户 frame、锁内一次提交，
  ...
  LA64 308/314；custom handler 投递侧的锁外用户栈写入留给 B47。
