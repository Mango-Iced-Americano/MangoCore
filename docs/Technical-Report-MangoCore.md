...
- [1.1 项目背景](#11-项目背景)
- [1.2 系统设计目标](#12-系统设计目标)
- [1.3 MangoCore总体演进路线](#13-mangocore总体演进路线)
- [1.4 关键设计](#14-关键设计)
- [1.5 整体技术路线](#15-整体技术路线)
- [1.6 项目贡献统计](#16-项目贡献统计)
- [1.7 本章小结](#17-本章小结)
- [2.1 系统设计理念](#21-系统设计理念)
- [2.2 MangoCore发展历程](#22-mangocore发展历程)
- [2.3 系统总体架构](#23-系统总体架构)
- [2.4 模块解耦设计思想](#24-模块解耦设计思想)
- [2.5 Boot启动流程](#25-boot启动流程)
- [2.6 文件系统总体架构](#26-文件系统总体架构)
- [2.7 网络总体架构](#27-网络总体架构)
- [2.8 内存管理总体架构](#28-内存管理总体架构)
- [2.9 Benchmark开发体系](#29-benchmark开发体系)
- [2.10 本章小结](#210-本章小结)
- [3.1 设计背景](#31-设计背景)
- [3.2 工程开发总体架构](#32-工程开发总体架构)
- [3.3 Docker统一开发环境](#33-docker统一开发环境)
- [3.4 多平台交叉编译体系](#34-多平台交叉编译体系)
- [3.5 Git协同开发体系](#35-git协同开发体系)
- [3.6 调试平台设计](#36-调试平台设计)
- [3.7 本章小结](#37-本章小结)
- [4.1 内核设计背景](#41-内核设计背景)
- [4.2 Task设计](#42-task设计)
- [4.3 后台维护逻辑](#43-后台维护逻辑)
- [4.4 Task生命周期](#44-task生命周期)
- [4.5 调度路径设计](#45-调度路径设计)
- [4.6 Trap与系统调用机制](#46-trap与系统调用机制)
- [4.7 Signal机制设计](#47-signal机制设计)
- [4.8 本章小结](#48-本章小结)
- [5.1 设计背景](#51-设计背景)
- [5.2 MangoCore内存管理演进过程](#52-mangocore内存管理演进过程)
- [5.3 内存管理总体架构](#53-内存管理总体架构)
- [5.4 StackFrameAllocator 物理页管理](#54-stackframeallocator-物理页管理)
  - [5.4.1 设计原理](#541-设计原理)
  - [5.4.2 分配流程](#542-分配流程)
  - [5.4.3 释放流程](#543-释放流程)
  - [5.4.4 OOM Handler（feature-gated）](#544-oom-handlerfeature-gated)
- [5.5 Kernel Heap设计](#55-kernel-heap设计)
  - [5.5.1 设计原理](#551-设计原理)
  - [5.5.2 堆空间初始化](#552-堆空间初始化)
  - [5.5.3 分配流程](#553-分配流程)
- [5.6 虚拟内存管理机制设计](#56-虚拟内存管理机制设计)
  - [5.6.1 AddressSpace 设计](#561-addressspace-设计)
  - [5.6.2 VmaSet 与 Vma 设计](#562-vmaset-与-vma-设计)
  - [5.6.3 VmaSet 管理](#563-vmaset-管理)
- [5.7 Page Fault 与文件映射缺页处理](#57-page-fault-与文件映射缺页处理)
  - [5.7.1 FaultAction 枚举](#571-faultaction-枚举)
  - [5.7.2 Page Fault 处理流程](#572-page-fault-处理流程)
  - [5.7.3 CoW 实现](#573-cow-实现)
- [5.8 mmap统一文件映射机制](#58-mmap统一文件映射机制)
  - [5.8.1 mmap 流程](#581-mmap-流程)
  - [5.8.2 Lazy Mapping 策略](#582-lazy-mapping-策略)
  - [5.8.3 MAP_PRIVATE 与 CoW](#583-map_private-与-cow)
- [5.9 交换与压缩机制（feature-gated）](#59-交换与压缩机制feature-gated)
  - [5.9.1 Zram 设计](#591-zram-设计)
  - [5.9.2 Swap 设计](#592-swap-设计)
- [5.10 TLB 管理](#510-tlb-管理)
  - [5.10.1 TLB 刷新机制](#5101-tlb-刷新机制)
  - [5.10.2 刷新时机](#5102-刷新时机)
- [5.11 本章小结](#511-本章小结)
- [6.1 设计背景](#61-设计背景)
- [6.2 文件系统总体架构](#62-文件系统总体架构)
- [6.3 Virtual File System 设计](#63-virtual-file-system-设计)
  - [6.3.1 File 层](#631-file-层)
  - [6.3.2 IndexNode 层](#632-indexnode-层)
  - [6.3.3 FileSystem 层](#633-filesystem-层)
  - [6.3.4 整体关系](#634-整体关系)
- [6.4 Mount 管理机制](#64-mount-管理机制)
  - [6.4.1 MountFS 与 MountList 结构](#641-mountfs-与-mountlist-结构)
  - [6.4.2 文件查找流程](#642-文件查找流程)
- [6.5 PageCache 缓存设计](#65-pagecache-缓存设计)
  - [6.5.1 核心结构](#651-核心结构)
  - [6.5.2 访问流程](#652-访问流程)
  - [6.5.3 Dirty 跟踪与 WriteBack](#653-dirty-跟踪与-writeback)
  - [6.5.4 PageEntry 状态机](#654-pageentry-状态机)
- [6.6 ext4 三层缓存架构](#66-ext4-三层缓存架构)
  - [6.6.1 第一层：PageCache（文件数据）](#661-第一层pagecache文件数据)
  - [6.6.2 第二层：inode_cache + Dirty Inode Flush](#662-第二层inode_cache-dirty-inode-flush)
  - [6.6.3 第三层：MetaBlockCache（元数据块）](#663-第三层metablockcache元数据块)
- [6.7 文件访问流程分析](#67-文件访问流程分析)
- [6.8 本章小结](#68-本章小结)
- [7.1 设计背景](#71-设计背景)
- [7.2 网络总体架构](#72-网络总体架构)
  - [7.2.1 路由层](#721-路由层)
- [7.3 smoltcp 协议栈集成](#73-smoltcp-协议栈集成)
  - [7.3.1 依赖声明](#731-依赖声明)
  - [7.3.2 Interface 与 SocketSet](#732-interface-与-socketset)
  - [7.3.3 协议支持](#733-协议支持)
- [7.4 Socket统一抽象](#74-socket统一抽象)
  - [7.4.1 Socket 类型](#741-socket-类型)
  - [7.4.2 SocketFile](#742-socketfile)
  - [7.4.3 Unix Domain Socket](#743-unix-domain-socket)
  - [7.4.4 socketpair](#744-socketpair)
- [7.5 RouteSocketHandle设计](#75-routesockethandle设计)
  - [7.5.1 核心定义](#751-核心定义)
  - [7.5.2 设计思想](#752-设计思想)
- [7.6 SocketBinding机制](#76-socketbinding机制)
  - [7.6.1 绑定流程](#761-绑定流程)
- [7.7 DeviceStack设计](#77-devicestack设计)
  - [7.7.1 核心结构](#771-核心结构)
  - [7.7.2 职责划分](#772-职责划分)
- [7.8 TCP Fast Path 优化](#78-tcp-fast-path-优化)
  - [7.8.1 实际实现](#781-实际实现)
- [7.9 PortManager 端口管理](#79-portmanager-端口管理)
- [7.10 网络数据流流程](#710-网络数据流流程)
  - [7.10.1 发送流程](#7101-发送流程)
  - [7.10.2 接收流程](#7102-接收流程)
- [7.11 本章小结](#711-本章小结)
- [8.1 工程优化设计背景](#81-工程优化设计背景)
- [8.2 调试总体架构](#82-调试总体架构)
- [8.3 trace ring 跟踪机制](#83-trace-ring-跟踪机制)
  - [8.3.1 设计目的](#831-设计目的)
  - [8.3.2 核心特性](#832-核心特性)
  - [8.3.3 实现范围](#833-实现范围)
- [8.4 heap_trace 内存分析工具](#84-heap_trace-内存分析工具)
  - [8.4.1 核心功能](#841-核心功能)
  - [8.4.2 实际输出内容](#842-实际输出内容)
  - [8.4.3 heap_trace 的限制](#843-heap_trace-的限制)
  - [8.4.4 适用范围](#844-适用范围)
- [8.5 procfs 调试框架](#85-procfs-调试框架)
- [8.6 sysfs 运行状态管理](#86-sysfs-运行状态管理)
  - [8.6.1 注册路径](#861-注册路径)
  - [8.6.2 Feature-gated 路径](#862-feature-gated-路径)
  - [8.6.3 路径可用性](#863-路径可用性)
- [8.7 性能统计与任务资源信息](#87-性能统计与任务资源信息)
  - [8.7.1 可统计的指标](#871-可统计的指标)
  - [8.7.2 导出方式](#872-导出方式)
- [8.8 Zombie 任务检测](#88-zombie-任务检测)
  - [8.8.1 Zombie 队列](#881-zombie-队列)
  - [8.8.2 Zombie 处理](#882-zombie-处理)
  - [8.8.3 覆盖范围](#883-覆盖范围)
  - [8.8.4 Zombie PCB 完整回收案例](#884-zombie-pcb-完整回收案例)
- [8.9 Panic Diagnostics 与 OOM 处理](#89-panic-diagnostics-与-oom-处理)
  - [8.9.1 Panic 处理](#891-panic-处理)
  - [8.9.2 OOM 处理](#892-oom-处理)
- [8.10 本章小结](#810-本章小结)
- [9.1 Benchmark设计理念](#91-benchmark设计理念)
- [9.2 实验平台与测试环境](#92-实验平台与测试环境)
  - [9.2.1 测试环境](#921-测试环境)
  - [9.2.2 测试配置](#922-测试配置)
  - [9.2.3 测试归档](#923-测试归档)
- [9.3 Benchmark总体框架](#93-benchmark总体框架)
- [9.4 Linux兼容性验证](#94-linux兼容性验证)
  - [9.4.1 BusyBox 验证](#941-busybox-验证)
  - [9.4.2 libc-test 验证](#942-libc-test-验证)
  - [9.4.3 Lua 运行验证](#943-lua-运行验证)
- [9.5 文件系统 Benchmark 分析](#95-文件系统-benchmark-分析)
  - [9.5.1 测试内容](#951-测试内容)
  - [9.5.2 测试结果](#952-测试结果)
  - [9.5.3 PageCache 说明](#953-pagecache-说明)
- [9.6 网络 Benchmark 分析](#96-网络-benchmark-分析)
- [9.7 回归验证体系](#97-回归验证体系)
  - [9.7.1 验证流程](#971-验证流程)
  - [9.7.2 说明](#972-说明)
- [9.8 本章小结](#98-本章小结)
- [10.1 项目整体回顾](#101-项目整体回顾)
- [10.2 当前内核架构总览](#102-当前内核架构总览)
- [10.3 主要实现内容](#103-主要实现内容)
  - [10.3.1 双架构 Rust no_std 内核](#1031-双架构-rust-no_std-内核)
  - [10.3.2 Linux 系统调用兼容面扩展](#1032-linux-系统调用兼容面扩展)
  - [10.3.3 文件系统](#1033-文件系统)
  - [10.3.4 网络系统](#1034-网络系统)
  - [10.3.5 内存管理](#1035-内存管理)
  - [10.3.6 进程管理](#1036-进程管理)
  - [10.3.7 事件通知机制](#1037-事件通知机制)
  - [10.3.8 调试与观测体系](#1038-调试与观测体系)
  - [10.3.9 测试基础设施](#1039-测试基础设施)
- [10.4 与早期版本的主要差异](#104-与早期版本的主要差异)
- [10.5 第三方依赖与参考](#105-第三方依赖与参考)
- [10.6 后续发展规划](#106-后续发展规划)
  - [10.7.1 已实现](#1071-已实现)
  - [10.7.2 部分实现](#1072-部分实现)
  - [10.7.3 未实现](#1073-未实现)
  - [10.7.4 设想](#1074-设想)
- [10.8 核心设计原则总结](#108-核心设计原则总结)
  - [统一抽象](#统一抽象)
  - [模块解耦](#模块解耦)
  - [测试驱动验证](#测试驱动验证)
  - [工程化开发](#工程化开发)
