...
调度器位于 `task/manager.rs` 和 `task/processor.rs`，使用单核 ready queue、interruptible queue 和 zombie queue。默认 nice 为 0 的路径按 FIFO 取任务；存在非零 nice 任务时进入简化的公平选择路径。
...
| 调度队列和等待队列 | `os/src/task/manager.rs` |
...
| ready queue | interruptible queue | zombie queue | idle ctx |
+-------------------------------------------------------------+
...
| `TaskManager` | 全局调度器 | ready/interruptible/zombie 队列管理、唤醒、移除、nice-aware fetch |
| `Processor` | 当前 CPU | 当前任务、idle task context、调度主循环 |
...
| `scheduler.md` | ready/interruptible/zombie 队列、fetch 策略、主循环 |
