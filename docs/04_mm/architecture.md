...
```
copy_from_user() / copy_to_user() / translated_str() / UserBuffer
    check range and current token
    for each page:
        lock current AddressSpace
        resolve current PTE, fault-in only when needed
        permission post-check
        copy through direct-map raw pointer
        unlock before allocation/parsing/next page

UserBuffer representation
    contiguous VA range or one VA range per iovec
    never retain PA, direct-map pointer, frame, or Rust slice
    actual copy revalidates each page under the VM lock
```

标量、数组、字符串和 buffer 都允许跨页，并逐页在 VM 锁内复制。单次 buffer 有 8 MiB
上限，iovec 数量上限为 1024。`fault_in_user_range()` 只用于副作用前预检查，真正访问仍会
再验证当前映射；已有 PTE 满足权限时 resolve-first fast path 不进入 fault handler。
