...
tags: [mm, frame, pagetable, vm-page-store]
---
...
1. `vmas.clear_no_hole()` 释放 VMA 和 resident frame 引用。
2. `page_table.release_frames()` 释放页表自身持有的页表页。

仅清除 VMA 不足以释放页表页；仅释放页表页也不能 drop 用户页帧引用。两个步骤都需要。

## 14. TLB 同步

页表 PTE 修改必须配套 TLB 失效。当前上层显式可见的批量场景是 fork：
...
其他 PTE 修改，如 `set_ppn()`、`set_user_flags()`、`unmap()`，依赖具体页表实现保证 TLB 一致。新增页表实现必须维护同样契约。
...
释放地址空间也要分两步：drop VMA/page store 释放用户 frame，release page table 释放页表页。只做前者会泄漏页表页，只做后者会让用户 frame 的 `Arc` 继续存在。
