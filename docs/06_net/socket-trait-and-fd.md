...
| `auto_bind_endpoint` | `fn auto_bind_endpoint(&self, peer, purpose) -> Result<Option<Endpoint>, SyscallErr>` | 未持 PortRegistry 时由 INET socket 推导候选 auto-bind 端点；已绑定/非 INET 返回 `None` |
