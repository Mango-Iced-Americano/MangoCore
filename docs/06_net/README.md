...
tags: [net, smoltcp, socket, tcp, udp]
---
...
| `os/src/net/config.rs` | NET_INTERFACE 静态变量，smoltcp Iface + SocketSet 管理 |
| `os/src/net/routing.rs` | FIB（转发信息库），路由查找 |
| `os/src/net/neighbour.rs` | 邻居缓存（ARP / NDP） |
| `os/src/net/adapter.rs` | SmoltcpDeviceAdapter trait 实现，轮询循环 |
| `os/src/net/router_device.rs` | 多接口路由的路由器设备 |
| `os/src/net/iface.rs` | 接口管理，地址配置 |
| `os/src/net/ioctl.rs` | SIOCGIF* ioctl 处理函数 |
| `os/src/net/macros.rs` | impl_file_for_socket! 及其他辅助宏 |
| `os/src/net/net_core.rs` | 网络核心初始化，DHCP 探测 |
...
| [smoltcp-device-routing.md](smoltcp-device-routing.md) | smoltcp 集成，适配器，轮询，FIB，ARP |
| [tcp.md](tcp.md) | TCP 状态机，connect/accept/数据流，epoll 事件 |
| [udp-raw-unix-netlink-packet.md](udp-raw-unix-netlink-packet.md) | UDP、RAW、Unix、Netlink、Packet 套接字详解 |
