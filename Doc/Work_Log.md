...
**备注：**
- 防御性 `unwrap_or` 回退保留硬编码 IP 作为最后兜底，当 net_core 查询不到接口（如过早初始化期）时使用
- 发现 `bind.rs:21` 和 `udp.rs:479` 已有 `net_core::default_iface()` + `unwrap_or` 模式（非本次新增）
- 模式：`crate::net::net_core::loopback_iface().and_then(|d| d.ip_addrs.first().map(|c| c.address())).unwrap_or(IpAddress::v4(127, 0, 0, 1))`

### 新增 NET_CORE LTP 测试组到 inet_test.rs

**涉及文件：**
- `user/src/bin/inet_test.rs` — 新增 LTP 风格测试宏（`tpass!`/`tfail!`/`tbrok!`/`tconf!`）、`errno_from_ret` 辅助函数、6 个 NET_CORE 测试用例，并集成到 `main()` 测试数组和汇总输出。

**验证：**
- `cargo build --bin inet_test`（user 目录）— 编译失败于 `user_lib` 预存错误（`src/syscall.rs`/`src/lib.rs`），非本次引入，inet_test.rs 本身无编译错误。
- 语法审查：6 个测试函数全部通过人工语法检查（花括号/括号平衡、返回值类型、资源清理路径）。

**备注：**
- 6 个测试用例覆盖：接口存在性（lo/eth0）、回环路由、路由查找、临时端口范围、端口绑定冲突、端口释放后重用。
- LTP 宏使用 `format_args!` + `println!` 输出，兼容现有 no_std user_lib 输出风格。
- 原有 11 个测试用例未做修改，仅扩展测试数组长度（11 → 17）。
- 修复了 `test_https_download` 中预存的格式字符串笔误 `({}/{)` → `({}/{})`。

### 新增路由表基础类型（RouteType / RouteEntry / RouteTable）

### 新增路由表基础类型（RouteType / RouteEntry / RouteTable）

**涉及文件：**
- `os/src/net/routing.rs` — 新建文件，定义 `RouteType(Connected/Static/Default)`、`RouteEntry`、`RouteTable` 类型及 `new()`/`add()`/`remove()` 方法。
- `os/src/net/mod.rs` — 添加 `pub mod routing;` 导出路由模块。

**验证：**
- `cd os && cargo check` ✅（仅 lang_items.rs 预存错误，无新引入错误）

**备注：**
- 使用 `smoltcp::wire::{IpAddress, IpCidr}` 作为地址/路由前缀类型，与现有网络栈一致。
- RouteTable 为纯结构体，未使用 spin::Mutex 封装（并发由上层调用者保证）。
- lookup_route() 和 Router 结构体留待后续任务实现。
